...
- 외부 provider, 인증, 결제, webhook, 배포 플랫폼이 포함된 작업은 일반 기능과 다르게 취급합니다.
- 이런 작업은 구현 전에 최소한 아래 문서가 프로젝트 맞춤형 내용으로 존재해야 합니다.
  - `docs/01-product.md`
  - `docs/02-system-architecture.md`
  - `docs/03-engineering-rules.md`
  - 관련 기능 스펙 `docs/04-features/<feature>/04-spec.md`
  - `docs/05-test-strategy.md`
- 외부 provider 또는 배포 플랫폼 제약이 핵심이라면 관련 ADR과 런북도 구현 전에 함께 준비해야 합니다.
- 로컬 데모 또는 스캐폴드는 `통합 완료`로 간주하지 않습니다.
- 외부 통합 작업은 hosted 환경에서의 실제 검증 증거가 없으면 완료로 간주하지 않습니다.
