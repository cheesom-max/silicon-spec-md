...
## 배포 및 provider 준비 체크리스트
- 필요한 provider/project가 실제 enabled 상태인지 확인한다.
- callback / redirect / webhook URL이 환경별로 정확히 등록됐는지 확인한다.
- 환경변수와 시크릿 owner가 누구인지 확인한다.
- deploy platform blocker가 없는지 확인한다. 예: git identity, 팀 정책, 권한 제한.
