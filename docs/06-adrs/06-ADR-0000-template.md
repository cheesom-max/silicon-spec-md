# ADR-0000: 제목

## 목적
- 하나의 지속되는 아키텍처 결정을 기록한다.

## 사용 순서
- 6단계: 오래 유지될 기술 결정이 생겼을 때 작성한다.
- 기능 스펙이나 아키텍처 문서만으로 결정 배경을 보존하기 어려울 때 사용한다.

## 대상 독자
- 어떤 아키텍처 선택이 왜 내려졌는지 이해해야 하는 엔지니어와 AI 에이전트.

## 범위 밖
- 상시 유지되는 아키텍처 설명.
- 기능 동작.
- 운영 절차.

## 정본으로 다루는 범위
- 하나의 지속되는 결정에 대한 맥락, 결정 내용, 상태, 결과, 대안.

## 마지막 업데이트 시점
- 결정 상태가 바뀌거나 이 기록이 대체될 때.

## 정본 링크
- 현재 시스템 구조는 `docs/02-system-architecture.md`를 참고한다.
- 관련 기능 스펙이나 런북이 있으면 함께 연결한다.

## 상태
- 제안됨 | 수락됨 | 폐기됨 | 대체됨

## 맥락
- 어떤 힘, 제약, 문제 때문에 이 결정을 내리게 되었는가?
- 이 결정이 인증, 인가, 공개 엔드포인트 보호, Storage 접근, abuse 방지에 영향을 주는가?

## 결정
- 무엇을 결정했는가?
- 보안 경계, 권한 모델, 시크릿 사용 경로, 공개 접근 정책이 함께 바뀐다면 그 기준을 명시한다.

## 실행 메타데이터
- 이 결정이 직접 영향을 미치는 범위는 무엇인가?
- 적용 전에 확인해야 할 전제 조건은 무엇인가?
- 적용 이후 관찰해야 할 핵심 신호는 무엇인가?
- 서명 검증 실패, 권한 거절, rate limit 발동, abuse 급증 같은 보안 신호가 있다면 함께 적는다.

## 결과
- 긍정적 결과.
- 부정적 결과.
- 이 결정으로 인해 생긴 후속 작업.
- 보안상 새로 줄어드는 위험과 새로 생기는 운영 부담이 있다면 함께 적는다.

## 고려한 대안
- 어떤 다른 선택지를 검토했고, 왜 채택하지 않았는가?
- 플랫폼 기본값에만 맡기는 방안, 소유권 검증을 생략하는 방안, 공개 접근을 넓히는 방안 같은 보안 대안도 포함한다.

## 대체 관계
- 필요하면 관련 ADR을 연결한다.
